...
public:
    ...
    void draw(Renderer& renderer, Light& L, float ka, float kd) {
        drawBand(renderer, L, ka, kd, 0, renderer.canvas.getHeight());
    }

    // Draw the triangle on a vertical band [yStart, yEnd)
    // Input Variables:
    // - yStart, yEnd: Inclusive/exclusive scanline bounds used for multi-threaded rendering
    void drawBand(Renderer& renderer, Light& L, float ka, float kd, int yStart, int yEnd) {
        vec2D minV, maxV;
        ...
        const int minY = std::max(static_cast<int>(minV.y), yStart);
        const int maxY = std::min(static_cast<int>(std::ceil(maxV.y)), yEnd);
